...
# os.environ["PYTHONPATH"] = str(Path(__file__).parent.parent.parent / "satclip" / "satclip") + ":" + os.getenv("PYTHONPATH", "")
...
class LocMatchESRGANModel(SRGANModel):
    """
    ...
    def __init__(self, opt):
        super(LocMatchESRGANModel, self).__init__(opt)
    ...
    def feed_data(self, data):
        ...
        if 'coords' in data:
            self.coords = data['coords'].to(self.device).float()
        else:
            self.coords = []
    ...
    def optimize_parameters(self, current_iter):
        ...
        if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
            ...
            fake_g_pred, obj_pred = self.net_d.osm_obj(disc_input, gen_objs)
            ...
            # --- Location Matching Loss ---
            if hasattr(self.net_d, 'loc_matching') and hasattr(self, 'coords') and self.coords is not None and len(
                    self.coords) > 0:
                coords = self.coords  # (B, 2)
                B = coords.size(0)
                ...
                # Real HR with false location
                real_false_score = self.net_d.loc_matching(gan_gt, false_loc_emb)
                # Generated with false location
                fake_false_score = self.net_d.loc_matching(self.output, false_loc_emb)

                l_loc_match = (
                        F.binary_cross_entropy_with_logits(real_score, torch.ones_like(real_score)) +
                        F.binary_cross_entropy_with_logits(real_false_score, torch.zeros_like(real_false_score)) +
                        F.binary_cross_entropy_with_logits(fake_false_score, torch.zeros_like(fake_false_score))
                )

                lambda_loc_match = self.opt.get('lambda_loc_match', 1.0)
                l_g_total += lambda_loc_match * l_loc_match
                loss_dict['l_loc_match'] = l_loc_match
