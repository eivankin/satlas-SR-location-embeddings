"""
FIXME: that is a version that selects images instead of the whole tiles (groups of images). See 'select_tiles_by_worldcover.py' for the other version.
...
3. Selects images with >30% urban coverage.
...
SELECTED_TILES_OUTPUT = Path("./selected_images.csv")
...
def process_single_tar(tar_file: Path):
    """Process a single tar file and return list of selected tiles"""
    selected_tiles = []
    ...
    try:
        for png_file in extract_dir.glob("worldcover/*.png"):
            ...
            if urban_coverage >= MIN_URBAN_COV:
                img_parts = png_file.stem.split('_')
                image_row, image_col = img_parts[1], img_parts[2]
...
def process_tiles(max_workers=None):
    ...
    with mp.Pool(processes=max_workers, initializer=init_worker) as pool:
        try:
            # Process files in parallel using imap_unordered for better performance
            results_iter = pool.imap_unordered(process_single_tar, tar_files)

            # Collect results with progress bar
            for selected_tiles in tqdm(results_iter,
                                     total=len(tar_files),
                                     desc="Processing WorldCover tiles"):
                all_selected_tiles.extend(selected_tiles)
    ...
    return pd.DataFrame(all_selected_tiles)
...
if __name__ == '__main__':
    try:
        ...
        print(f"Selected {len(selected_df)} tiles with >{MIN_URBAN_COV}% urban coverage") # Selected 281831 image tiles with >30% urban coverage
